...
check_os() {
    ...
    check_configure_dns
    echo "$(date "+%Y-%m-%d %H:%M:%S") 开始安装Wget..."
    sudo yum install -y wget &> /dev/null
    echo "$(date "+%Y-%m-%d %H:%M:%S") Wget已安装"
    # 校准时间时区
...
reboot_os() {
    echo
    _info "系统已进行内核升级，重启即可开启BBR加速."
    read -p "是否进行重启? [y/n]" is_reboot
